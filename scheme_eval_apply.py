...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in scheme_forms.SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 3
        return scheme_apply(scheme_eval(expr.first, env), expr.rest.map(lambda arg: scheme_eval(arg, env)), env)
        # END PROBLEM 3

def pair2list(args,lst=[]):
    if args is not nil:
        return lst + [args.first] + pair2list(args.rest)
    return []
